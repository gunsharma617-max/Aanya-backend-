...
function checkOrigin(request: Request): void {
  ...
  // Requests without Origin support server-to-server clients and curl.
...
function checkRateLimit(
  ...
  // Trust this header only behind a proxy that overwrites it, such as
  // the intended Vercel deployment. Do not trust arbitrary client
  // forwarding headers on an unprotected self-hosted server.
  const ip =
  ...
  if (!bucket || bucket.resetAt <= now) {
    if (!bucket && buckets.size >= MAX_BUCKETS) {
      throw new HttpError(429, "Too many requests. Please try again later.", 60);
    }
...
export function preflight(request: Request): Response {
  try {
    ...
    return errorResponse(error, "json", "Request could not be processed.");
  }
}
...
export function errorResponse(
  ...
  // Only explicitly constructed, safe errors can supply public text.
  // Never serialize an arbitrary exception or a provider response.
